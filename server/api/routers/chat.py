...
def build_system_prompt(profiles: list[dict[str, Any]] | None = None) -> str:
    ...
    system_prompt += (
        ...
        "\n\nIMPORTANT — locale restriction: the sandbox runs with only the "
        "C/POSIX locale. NEVER use locale.setlocale (it raises locale.Error "
        "for any non-C locale) or any other locale-dependent formatting. "
        "If the answer needs localized names (e.g. weekday or month names "
        "in Spanish), map them yourself with a plain list or dict of names."
        ...
        "figures (variables named fig, fig1, fig2...), ONE result table "
        "(a DataFrame named df_result or df_<name>) and printed output. "
        "The user NEVER sees console output unless it is printed or "
        "assigned to such variables. So: (1) assign the single result "
        "table to a DataFrame variable named df_result (or one "
        "df_<name>); if the question seems to need several tables, still "
        "surface ONLY the main result as df_result and give any other "
        "frame a non-df name so it never renders; (2) print() the key "
        "metrics; (3) state the main numbers directly in 'explanation' — "
        "never just describe what the code computes. Load the raw dataset "
        "into 'df' and do NOT reassign 'df' with a filtered/aggregated "
        "result: use a new df_<name> variable instead, so the raw dataset "
